...
- 修复 2025-01 非日本作品因国家证据缺失而被低评分结果支配自动排除的问题；精确法国地区证据与用户裁决现在收敛到非日本硬拒绝，旧自动状态会事务性 reconciliation。
- 收紧 Movie 范围：正典 `剧场版` 中明确标注为游乐设施电影或天文馆上映的内容不再进入普通院线档案，缺少结构化证据的单独个案使用人工排除。

### 调整

- 同步报告新增 `auto_reconciled`，区分“硬拒绝取代旧自动排除”和“恢复收录”，并补充对应离线控制语料与幂等回归。
