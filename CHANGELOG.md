...
- 自动排除现在是可重新评估的缓存状态；相关季度刷新时，评分增长或证据补全可以安全恢复原本可收录的作品，并保持人工排除和范围规则不变。
- 收紧自动排除恢复的事务、幂等性和离线状态转换回放，避免旧配置在同步后的 build 中继续遮蔽已恢复事实。

### 调整

- 固化 REVIEW 的 ChatGPT 辅助人工流程，并要求 live-data Plan 在最终 live validation、review、audit 和回归完成后才进行正式 source push。

## 0.8.3 - 2026-08-27

### 新增

- REVIEW 现在提供独立的 `bgmb classify BGM_ID --japanese`、`--non-japanese` 和 `--clear` 人工日本性裁决，并将决定保存到独立的 Git-trackable 配置。

### 修复

- 先按自然季度与受限 TV 边界判定候选相关性；跨季度 Movie 搜索回看不会再产生日本性 REVIEW 或错误的季度分配指引。
- 日本性证据融合支持已核实的国家字段别名、`・` 分隔符与兼容的联合制作；可靠日期且低评分的未决日本性分支会记录为可审计的结果支配自动排除。

## 0.8.2 - 2026-08-27

### 修复

- 修复跨自然季度边界的 TV 首播归属：1～2 集短篇保留自然季度；只有具备连续多周正片播出证据和明确目标季度证据的长篇 TV 才允许进入下一季度。
- 季度边界证据不足现在保留为 REVIEW，不再触发永久自动黑名单，避免误伤并保留人工裁决路径。

## 0.8.1 - 2026-08-25

### 修复

- 在触摸界面使用标准 `touch-action: manipulation` 抑制 iOS Safari / PWA 的双击页面缩放，同时保留正常滚动与双指缩放，避免快速连续操作浏览控件时页面意外放大。

## 0.8.0 - 2026-08-24

### 新增

- 应用启动后会在后台自动检查并维护已经完整下载的季度；资料或离线包存在新版本时复用现有校验队列静默更新，无需逐个进入 Settings 手动更新。
- 离线启动不会阻塞应用；恢复联网后会补执行一次当前会话的后台检查。

### 调整

- 自动维护只作用于用户已经下载的季度，不会自行下载从未选择离线的历史季度；应用代码版本仍保持显式刷新，不会自动 reload。

## 0.7.3 - 2026-08-24

### 调整

- 移动菜单中的 Archive 改为“档案”，并收紧菜单宽度、区块间距与无意义空白，同时保留可靠触控尺寸和右上角锚定定位。

## 0.7.2 - 2026-08-24

### 修复

- 修复播出档案在手机端无视分页设置、始终渲染全部结果的问题；Archive 现在在所有屏幕尺寸下都严格使用当前页数据。

### 调整

- 播出档案分页大小统一为 10 / 20 / 50，默认保持 20；旧的 40 / 60 / 100 本地偏好自动回落到 20。

## 0.7.1 - 2026-08-24

### 修复

- 将离线季度的完整资源 revision 与结构化季度资料 data_revision 分离；CSS、JavaScript、模板等应用资源变化不再被误报为“季度资料有更新”。
- 已下载的旧版季度可从既有季度 JSON 资源哈希恢复 data_revision，不会因为升级到新 manifest 结构而无条件要求重新下载全部季度。

## 0.7.0 - 2026-08-24

### 新增

- 重建移动浏览动效体系：排序/筛选使用局部位置 FLIP，Grid/List 使用作品外框、封面、内容和评分的共享几何过渡，TV/MOVIE 与季度导航使用只作用于结果区域的分层切换。

### 调整

- 季度切换继续避免整页 cross-document snapshot；目标页使用正确首帧后执行方向型作品入场。

### 修复

- 修复简单 X/Y FLIP 无法覆盖 Grid/List 大幅布局变化、导致布局瞬间跳变的问题。

## 0.6.8 - 2026-08-24

### 修复

- 修复 LIST 首帧预渲染属性在 hydration 后仍持续覆盖运行时 Grid/List 状态、导致点击 GRID 无法真实切换的问题。

## 0.6.7 - 2026-08-24

### 修复

- 修复移动端主菜单进入 top layer 后未正确锚定触发按钮、在 Safari/PWA 中出现偏移和大面积占位的问题。

## 0.6.6 - 2026-08-24

### 调整

- Settings 的一级里程碑列表现在同时显示对应 `x.y.0` 的正式发布日期；里程碑日期直接来源于具体版本记录。
- 新的 version-bearing CHANGELOG 条目必须包含明确发布日期，避免未来里程碑缺少日期。

## 0.6.5 - 2026-08-23

### 修复

- 移除会在 iOS Safari / standalone PWA 中产生整屏黑闪的 root 与跨页面 View Transition；TV/MOVIE、Grid/List、排序和筛选现在只对实际变化的作品结果使用局部位置与入场动画。

## 0.6.4 - 2026-08-23

### 修复

- 修复移动端主菜单被搜索框、季度导航和作品内容覆盖的问题；菜单现在使用可靠的顶层浮层语义，并与其他控制浮层互斥。

## 0.6.3 - 2026-08-23

### 新增

- Settings 更新日志改为里程碑分组：已完成的 `x.y.0` 作为默认收起的 `x.y` 分组，展开后直接展示该开发窗口内所有具体版本。
- 最新里程碑之后的 patch 版本在下一里程碑出现前直接展示在日志顶部，并在未来 milestone 出现后自动归组。

### 调整

- 删除“尚未发布”版本；用户可见变更从 commit 时直接获得具体版本号。
- 程序版本从“Plan/发布驱动”调整为“每个 Phase commit 独立判断 version impact”，多个程序版本允许在一次 Plan push/publish 中共同进入历史。

## 0.6.2 - 2026-08-23

### 修复

- Quarter 静态 HTML 直接按默认评分排序生成 TV/MOVIE，避免数据加载完成后首播与续播再发生一次可见重排。
...
- 初始化 Bangumi Side B 项目、可安装 Python 包与本地命令行基础。
