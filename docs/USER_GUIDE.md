...
手机详情是覆盖当前视口的全屏层。顶部返回、浏览器 history back 和主屏幕 PWA 的左缘右滑都关闭同一详情入口，并恢复列表原来的筛选、排序和滚动位置；standalone PWA 的应用手势依次经过 possible-drag、dragging、cancel 或 commit 状态，取消时冻结的背景列表和详情滚动均保持不变。普通 Safari 优先使用浏览器原生 history 手势，standalone PWA 才启用应用内回弹手势。
