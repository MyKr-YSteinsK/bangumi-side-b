...
- 保存的 Grid / List 偏好会在页面首次绘制前恢复，因此切换季度时不会先闪现另一种布局。支持 View Transition 的浏览器会为排序、筛选、媒体和视图切换保留作品身份与位置连续性；不支持时仍使用相同的同步状态更新。首屏最多 10 个作品只播放一次轻微的渐入位移动效。
...
手机季度浏览以作品为首要内容：季度快速切换位于页面顶部，作品使用两列 2:3 封面卡片；季度页在手机上连续展示当前季度匹配结果，不显示桌面分页控件。筛选按钮打开底部 sheet，勾选只修改草稿，点击“显示 N 部”一次性应用，关闭或系统返回会取消草稿；排序菜单仍是一次点击立即生效。站点页脚统一显示 `Bangumi Side B · MyKr`，海报预览没有可见关闭按钮，点击遮罩或按 Escape 退出，点击海报本身不会关闭。
