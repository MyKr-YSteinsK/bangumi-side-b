...
Settings 页面包含直接构建进 `settings/index.html` 的 06 / CHANGELOG：显示当前程序版本、
最新里程碑之后尚未收口的 patch 会以 standalone release 直接显示；已经完成的
`major.minor.0` 版本会作为默认收起的 milestone group，并显示该 anchor 版本的正式日期，
展开后直接显示该开发窗口内的 concrete child releases。页面只使用一层折叠，child release
不能再单独折叠。新版本条目必须包含明确的 `YYYY-MM-DD` 日期；日志离线可读，不会运行时请求
GitHub 或读取外部 CHANGELOG 文件。
...
- 首页进入最新已构建季度。
- 季度页默认显示 TV；首播与续播共用一条连续列表，续播只显示“续播”标识，Movie 仍单独浏览。
- 搜索匹配标题、原名、结构化别名和 Bangumi Subject ID；支持来源与社区标签筛选。
- 筛选工作区会显示按当前搜索和其它条件计算的结果数；选项以可换行 chip 展示，已选条件可在活动筛选区单独移除，也可以清除全部。
- 排序提供评分和评分人数的升降序，结果可分页浏览；排序菜单是锚定触发按钮的独立浮层，不会把结果列表向下推。
- Quarter 与 Archive 的结果工具栏提供 Grid / List 视图切换。Grid 适合扫封面，List 保留小封面并以更高密度展示标题、原名、日期、来源和评分；选择会写入浏览器 localStorage，在刷新、切换季度和进入 Archive 后保持。
- 保存的 Grid / List 偏好会在页面首次绘制前恢复，因此切换季度时不会先闪现另一种布局。排序、筛选、媒体和视图切换只对当前可见作品结果使用有上限的局部位置连续动效；页面背景、header 和跨页面导航不参与截图式过渡，因此不会出现整屏黑闪。首屏最多 10 个作品只播放一次轻微的渐入位移动效；`prefers-reduced-motion: reduce` 时直接更新状态，不执行位置或 stagger 动画。
- 手机 Grid 的季度日期使用 `MM-DD`、Archive 日期使用 `YY-MM-DD`，元信息保持单行；List 和桌面视图继续显示完整日期。
- Archive 可按季度、年份或年份范围查看已生成资料。
- 桌面端在列表右侧打开 detail / filter workspace；手机端选择作品或筛选时切换到不保留 context rail 的全宽单栏工作区，详情顶部使用紧凑返回箭头回到列表。
- URL 中的 `#bgm-ID` 可直达当前浏览范围内的作品详情，并支持浏览器前进/后退。

手机季度浏览以作品为首要内容：季度快速切换位于页面顶部，作品使用两列 2:3 封面卡片；季度页在手机上连续展示当前季度匹配结果，不显示桌面分页控件。主菜单使用 top-layer popover（旧浏览器 fallback 为 fixed 浮层），背景不透明、不会被搜索框或作品覆盖，点击外部区域或 Escape 关闭，并与排序、季度和筛选浮层互斥。筛选按钮打开底部 sheet，勾选只修改草稿，点击“显示 N 部”一次性应用，关闭或系统返回会取消草稿；排序菜单仍是一次点击立即生效。站点页脚统一显示 `Bangumi Side B · MyKr`，海报预览没有可见关闭按钮，点击遮罩或按 Escape 退出，点击海报本身不会关闭。

手机详情是覆盖当前视口的全屏层，打开时会冻结背景文档和作品列表，短详情不会产生无效滚动，长详情只滚动自身。顶部返回、浏览器 history back 和主屏幕 PWA 的左缘右滑都关闭同一详情入口，并恢复列表原来的筛选、排序和滚动位置；standalone PWA 的应用手势依次经过 possible-drag、dragging、cancel 或 commit 状态，起始小距离、取消和提交都只移动完整的前景详情层，不重复触发 history。普通 Safari 优先使用浏览器原生 history 手势，standalone PWA 才启用应用内回弹手势。详情内的海报预览使用页面级半透明遮罩，点击遮罩或按 Escape 关闭，点击海报本身不会关闭，也不会请求额外远程资源。
...
下载队列不会阻塞页面的其它操作，进度变化只更新当前任务和必要的季度状态；选择器、菜单和焦点不会因每个资源完成而重建。普通界面使用“未下载、下载中、已暂停、等待网络、下载未完成、已下载、有更新”等状态，暂停、继续、取消、重试和联网恢复均是明确操作；Service Worker 和 `INCOMPLETE` 等技术细节只在高级诊断中显示。
