...
`excluded_subject_ids` 是人工维护的永久排除；`auto_excluded_subject_ids` 是同步根据确定性规则自动追加的永久排除。规则 A 是可靠首播日期超过 7 天且评分人数少于 30；缺少任一可靠字段时不猜测。规则 B 只针对明确 allowlist 中的缺失证据型 REVIEW：目标季度结束超过 7 天，且评分人数少于 30 或缺失；评分人数缺失只在规则 B 中作为 low-signal。季度未成熟、评分人数达到 30、目标季度不明确以及任何冲突型 REVIEW 都不会触发规则 B。自动条目保留标题注释并记录审计证据；评分人数后来上涨也不会自动恢复，只有人工从自动列表删除 ID 后，后续 sync 才会重新评估。
