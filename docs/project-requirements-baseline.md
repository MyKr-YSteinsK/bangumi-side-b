...
作品必须有已验证的结构化 Infobox 国家/地区字段。仅当一致字段值的精确 token 包含
`日本` 或 `Japan` 时收录；同一值中的合拍国家允许。缺失、无法解析或多个字段 token
集合冲突时排除并审计，绝不以标题、简介、语言、公司或社区标签推断国家。
...
退出码非零；Ctrl+C 停止新请求并保证当前事务完成或回滚。只有通过格式、日期和国家过滤
的作品才请求正片章节和封面；不请求角色、人物、角色图片或续播刷新。
