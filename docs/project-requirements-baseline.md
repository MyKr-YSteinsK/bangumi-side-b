...
`excluded_subject_ids` 是人工维护的永久排除；`auto_excluded_subject_ids` 是同步根据可靠首播日期和评分人数自动追加的永久排除。自动规则固定为首播超过 7 天且评分人数少于 30，缺少任一可靠字段时不猜测。自动条目保留标题注释并记录审计证据；评分人数后来上涨也不会自动恢复，只有人工从自动列表删除 ID 后，后续 sync 才会重新评估。
