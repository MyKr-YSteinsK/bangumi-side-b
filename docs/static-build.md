...
deterministic shared, quarter, year, archive, and artifact fingerprints. A
changed quarter dirties its page/data/offline manifest, the owning year catalog,
and the archive index. Missing or corrupt state is treated as safely dirty. The
writer stages only changed files in `workspace/build-staging`, validates the
complete desired mapping, backs up touched files, applies replacements/deletions,
validates the final tree, and commits build-state last. A failed replace, delete,
or validation restores the touched files and leaves the previous state intact.
...
commit. Incomplete facts retain the last-known-good site; complete facts with
